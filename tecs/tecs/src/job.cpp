...
JobType::JobType(std::string name, uint32_t id, uint32_t priority, uint32_t cost) :
    name_(std::move(name)),
    id_(id),
    priority_(priority),
    cost_(cost)
...
JobState::JobState(std::weak_ptr<bool> is_completed) :
    is_completed_(std::move(is_completed))
{
    assert(!is_completed_.expired() && "is_completed weak pointer must not be expired");
}

bool JobState::IsCompleted() const
{
    // Attempt to lock the weak pointer to get a shared pointer
    if (std::shared_ptr<bool> is_completed_shared = is_completed_.lock())
        return *is_completed_shared;

    // If the weak pointer cannot be locked, it means the job state has been destroyed
    // In this case, we can consider the job as completed
    return true;
}
